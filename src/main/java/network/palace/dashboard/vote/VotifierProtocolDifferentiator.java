...
import network.palace.dashboard.vote.protocol.Protocol1Decoder;
import network.palace.dashboard.vote.protocol.Protocol2Decoder;
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf buf, List<Object> list) throws Exception {
        ...
        if (readMagic == PROTOCOL_2_MAGIC) {
            ...
            if (!testMode) {
                ...
                ctx.pipeline().addAfter("protocol2StringDecoder", "protocol2VoteDecoder", new Protocol2Decoder());
            ...
            if (!testMode) {
                ctx.pipeline().addAfter("protocolDifferentiator", "protocol1Handler", new Protocol1Decoder());
                ctx.pipeline().remove(this);
            }
        }
    }
