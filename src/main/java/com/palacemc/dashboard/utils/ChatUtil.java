...
public class ChatUtil {
    ...
    public void chatEvent(PacketPlayerChat packet) {
        ...
        boolean isSquire = rank.getRankId() >= Rank.SQUIRE.getRankId();
        ...
        if (command) {
            if (!dashboard.getCommandUtil().handleCommand(player, msg.replaceFirst("/", ""))) {
                player.chat(msg);
            }
            return;
        }
        ...
        if (!isSquire) {
            ...
            if (msg.startsWith(":warn-")) {
                dashboard.getWarningUtil().handle(player, msg);
                return;
            }
        }
    ...
    public void sendChat(Player player, String msg) {
        ...
        String serverName = dashboard.getServer(player.getServer()).getServerType();

        if (serverName.startsWith("New")) {
            serverName = serverName.replaceAll("New", "");
        }
        ...
        if (dashboard.getServer(player.getServer()).isPark()) {
            ...
            for (Player tp : dashboard.getOnlinePlayers()) {
                ...
                if (dashboard.getServer(tp.getServer()).isPark()) {
                    String send = ChatColor.WHITE + "[" + ChatColor.GREEN + serverName + ChatColor.WHITE + "] " + message;
    ...
    private void advertMessage(String name, String msg) {
        UUID id = UUID.randomUUID();
        String response = "Please do not share links.";
        Warning warning = new Warning(id, name, msg, response, System.currentTimeMillis() + 300000);
